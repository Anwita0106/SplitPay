...
**Phase 1 — compute_net_balances** (`app/services/balance_math.py`, read through
`balance_service`). One pass over all expenses (add the full amount to the payer's
balance) and their splits (subtract each participant's share), then net out
`COMPLETED` settlements (the payer of a settlement owes less, the receiver is owed less). O(E) time where E = number of expense_split rows in
...
- **Stateless JWT auth can't be revoked before expiry** — accepted for
  ...
  tokens.
- **Migrations are forward-only** — Alembic versions the schema
  (`backend/alembic/versions`); the baseline migration is idempotent so it adopts
  databases created from the original `schema.sql`. Downgrades deliberately refuse
  to run rather than destroy data.
